...
namespace MahERP.DataModelLayer.Repository
{
    public class BranchRepository : IBranchRepository
    {
        ...
        public List<TaskCalendarViewModel> GetTasksForCalendarView(string userId, int? branchId = null, DateTime? startDate = null, DateTime? endDate = null)
        {
            try
            {
                // کوئری اصلی برای دریافت تسک‌های دارای تاریخ مهلت
                var tasksQuery = from ts in _context.TaskAssignment_Tbl
                                ...
                                join stakeholder in _context.Stakeholder_Tbl on t.StakeholderId equals stakeholder.Id into stakeholderJoin
                                from sh in stakeholderJoin.DefaultIfEmpty()
                                join branch in _context.Branch_Tbl on t.BranchId equals branch.Id into branchJoin
                                from br in branchJoin.DefaultIfEmpty()
                                where ts.AssignedUserId == userId && 
                                      !t.IsDeleted && 
                                      t.DueDate.HasValue // فقط تسک‌های دارای تاریخ مهلت
                                select new { Task = t, Category = category, Stakeholder = sh, Branch = br };

                // فیلتر بر اساس شعبه در صورت ارسال
                if (branchId.HasValue)
                {
                    tasksQuery = tasksQuery.Where(x => x.Task.BranchId == branchId.Value);
                }

                // فیلتر بر اساس محدوده زمانی در صورت ارسال
                if (startDate.HasValue)
                {
                    tasksQuery = tasksQuery.Where(x => x.Task.DueDate >= startDate.Value);
                }

                if (endDate.HasValue)
                {
                    tasksQuery = tasksQuery.Where(x => x.Task.DueDate <= endDate.Value);
                }

                // تبدیل به لیست برای نمایش در تقویم
                var calendarTasks = tasksQuery.Select(x => new TaskCalendarViewModel
                {
                    Id = x.Task.Id,
                    Title = x.Task.Title,
                    ...
                    CompletionDate = x.Task.CompletionDate,
                    IsCompleted = x.Task.CompletionDate.HasValue,
                    CategoryTitle = x.Category != null ? x.Category.Title : "بدون دسته‌بندی",
                    CategoryId = x.Task.TaskCategoryId,
                    StakeholderName = x.Stakeholder != null ? 
                        (x.Stakeholder.FirstName + " " + x.Stakeholder.LastName).Trim() : 
                        "بدون طرف حساب",
                    StakeholderId = x.Task.StakeholderId,
                    BranchName = x.Branch != null ? x.Branch.Name : "بدون شعبه",
                    BranchId = x.Task.BranchId,
                    CreateDate = x.Task.CreateDate,
                    CreatorUserId = x.Task.CreatorUserId,
                    IsActive = x.Task.IsActive,
                    IsOverdue = x.Task.DueDate.HasValue && 
                               !x.Task.CompletionDate.HasValue && 
                               DateTime.Now > x.Task.DueDate.Value
            ...
            {
                // لاگ خطا
                throw new Exception($"خطا در دریافت تسک‌های تقویم برای کاربر {userId}: {ex.Message}", ex);
            }
        }
    }
}