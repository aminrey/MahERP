...
9. [Background Services و زمان‌بندی](#background-services-و-زمانبندی) ⭐ **به‌روزرسانی شده**
10. [نحوه راه‌اندازی](#نحوه-راهاندازی)
...
```
✅ مثبت:
🕐 زمان فعلی ایران: 2024-01-15 07:15:00
⏰ 1 قالب زمان‌بندی شده آماده اجرا است
📤 شروع ارسال قالب زمان‌بندی شده: اعلان روزانه
✅ قالب اعلان روزانه به 10 کاربر ارسال شد
📅 بروزرسانی زمان‌ها:
   LastExecutionDate: 2024-01-15 07:15:00
   NextExecutionDate: 2024-01-16 07:15:00
✅ اطلاعات زمان‌بندی برای اعلان روزانه بروزرسانی شد

⚠️ هشدار (در صورت وقوع):
⚠️ قالب اعلان روزانه در کمتر از 1 دقیقه پیش اجرا شده است. Skip.

❌ خطا:
❌ خطا در اجرای قالب زمان‌بندی شده #5 - اعلان روزانه
```
...
// وظایف:
✅ پردازش صف EmailQueue_Tbl
✅ ارسال ایمیل‌ها
✅ ثبت EmailLog
✅ Retry در صورت خطا (حداکثر 3 بار)
```

#### 📱 **4. SmsBackgroundService**
```csharp
// زمان‌بندی: مداوم
// فاصله چک: 20 ثانیه

// وظایف:
✅ پردازش صف SmsQueue_Tbl
✅ ارسال پیامک از طریق SmsProvider
✅ ثبت SmsLog
✅ بررسی وضعیت تحویل
```

#### ✈️ **5. TelegramPollingBackgroundService**
```csharp
// زمان‌بندی: مداوم
// فاصله چک: 5 ثانیه
...
#### 🔍 **6. ModuleTrackingBackgroundService**
```csharp
// زمان‌بندی: هر 5 دقیقه

// وظایف:
✅ ردیابی استفاده از ماژول‌ها
✅ ثبت آمار
✅ بررسی سلامت سیستم
```

#### 🧹 **7. ExpiredRoleCleanupBackgroundService**
```csharp
// زمان‌بندی: روزانه - 02:00 صبح

// وظایف:
✅ حذف نقش‌های منقضی شده
✅ پاکسازی دسترسی‌های موقت
✅ آرشیو لاگ‌های قدیمی
```

### تنظیمات زمان‌بندی قالب‌ها

#### الف) **روزانه (Daily)** - ScheduleType = 1
```csharp
NotificationTemplate
{
    ScheduleType = 1,
    ScheduledTime = "07:15",  // ساعت 7:15 صبح
    IsScheduleEnabled = true
}

// نتیجه:
// هر روز ساعت 7:15 صبح (به وقت ایران) اجرا می‌شود
```

#### ب) **هفتگی (Weekly)** - ScheduleType = 2
```csharp
NotificationTemplate
{
    ScheduleType = 2,
    ScheduledTime = "09:00",
    ScheduledDaysOfWeek = "1,3,5",  // دوشنبه، چهارشنبه، جمعه
    IsScheduleEnabled = true
}
...
#### ج) **ماهانه (Monthly)** - ScheduleType = 3
```csharp
NotificationTemplate
{
    ScheduleType = 3,
    ScheduledTime = "10:00",
    ScheduledDayOfMonth = 15,  // روز 15 هر ماه
    IsScheduleEnabled = true
}

// نتیجه:
// روز 15 هر ماه ساعت 10:00 صبح اجرا می‌شود
```
...
✅ CRM یکپارچه
✅ **سیستم اعلان‌رسانی چندکاناله با زمان‌بندی پیشرفته** ⭐ **به‌روزرسانی شده**
