    ...
    public class BaseController : Controller
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                // ⭐⭐⭐ بررسی دسترسی کاربر به ماژول CRM
                var userId = GetUserId();
                if (!string.IsNullOrEmpty(userId))
                {
                    var accessResult = _moduleAccessService.CheckUserModuleAccessAsync(userId, ModuleType.CRM).Result;
                    if (!accessResult.HasAccess)
                    {
                        TempData["ErrorMessage"] = "شما به ماژول مدیریت ارتباط با مشتری (CRM) دسترسی ندارید.";
                        context.Result = RedirectToAction("Index", "Dashboard", new { area = "AppCoreArea" });
                        return;
                    }
                }
