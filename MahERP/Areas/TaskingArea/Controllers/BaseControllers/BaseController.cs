    ...
    public class BaseController : Controller
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext context)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                // ⭐⭐⭐ بررسی فعال بودن ماژول Tasking در سیستم
                ...
                // ⭐⭐⭐ بررسی دسترسی کاربر به ماژول Tasking
                var userId = GetUserId();
                if (!string.IsNullOrEmpty(userId))
                {
                    var accessResult = _moduleAccessService.CheckUserModuleAccessAsync(userId, ModuleType.Tasking).Result;
                    if (!accessResult.HasAccess)
                    {
                        TempData["ErrorMessage"] = "شما به ماژول مدیریت تسک‌ها دسترسی ندارید.";
                        context.Result = RedirectToAction("Index", "Dashboard", new { area = "AppCoreArea" });
                        return;
                    }
                }
